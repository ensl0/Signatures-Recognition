...
class Signature():
    ...
    dataLength = 0
    # 30 30 - chosen because most blobs will fit in a 30 by 30 space
    INPUT_LENGTH = 900
    dataRows = []
    network = []
    alpha = 0.5
    numberHiddenLayers = 4
    nodesPerLayer = [900, 400, 200, 26]
    ...
    def __init__(self):
        ...
        # letters = self.segment(a)
        all_outputs, guess, spelling = self.evaluate_random_sample()
        for i, output in enumerate(all_outputs):
            correct_output = self.correct_letter_to_array(output[-1], spelling[i])
    ...
    def init_network(self, read):
        ...
        if read:
            ...
            for i in range(self.numberHiddenLayers):
                # weightmat = np.random.rand(self.nodesPerLayer[i], prev_outputs)
                # np.zeros(shape=(self.nodesPerLayer[i], prev_outputs))
                # np.random.rand(self.nodesPerLayer[i])
                weightmat = 2*(np.random.rand(self.nodesPerLayer[i], prev_outputs+1)-0.5)
                # biasvec = 2*(np.random.rand(self.nodesPerLayer[i])-0.5)

                #hidden_layers.append((weightmat, biasvec))
    ...
    def correct_letter_to_array(self, output, answer):
        # construct an array based on the answer
        actual = [0]*26
        e = ord(answer)
        actual[ord(answer) - 97] = 1
        error = np.array(actual) - np.array(output)
        return actual
    ...
    def backpropogation(self, output_list, expected_list):
        ...
        for i, layer in enumerate(self.network[::-1]):
            for j, node in enumerate(layer):
                ...
                for k, connected_node in enumerate(self.network[self.numberHiddenLayers - 1 - i]):
                    error += (connected_node * error_list[self.numberHiddenLayers - 1 - i][k])
                error_list[self.numberHiddenLayer - i][self.nodesPerLayer[self.numberHiddenLayer - j]]
    ...
    def segment(self, image):
        ...
        for i, box in enumerate(a):
            ...
            if (box[1].stop * box[0].start > 1 and box[1].start <= w - 1 and box[0].stop <= h - 1) or \
                ...
                b = len(cropped_letter)
                padded = np.pad(cropped_letter, (0, 900-len(cropped_letter)), mode='constant')
                letters.append(padded)
                # self.maxy = max(box_height, self.maxy)
                # self.maxx = max(box_width, self.maxx)
                # np.append(letters, (connected[box] > 0)*255)

        # Just some code to draw the regions found
